	...
	int fitness_test)
{
	...
	for (j = 0; j < num_outputs; j++)
	{
		...
		if (cgp_data < 1.0 / NUM_CLASSES) cgp_outputs[j] = 0.0; 
...
{
	printf("\nGENERATION is %d Best fitness is now %8.5lf which is %8.5lf %%", generation, fit, ((fit * 100) / (num_outputs * num_tests)));
}
	...
	int* best_chromosome)
{
	...
	fp = fopen(prog, "a");
	fprintf(fp, "\nGENERATION is %u     Best fitness is now %8.5lf with %8.5lf %%", gen, best_fit, ((best_fit * 100) / (num_outputs * num_tests)));
	fprintf(fp, "\nThe chromosome is\n");
	fclose(fp);
...
void run_test(char testfile[MAX_NUM_LETTERS], char outfile[MAX_NUM_LETTERS])
{
	...
	int i;
	read_data(testfile);
	chromosome = create_1d_space(num_genes);
	...
	fclose(fp);
	get_cgp_outputs(chromosome, outfile);
...
void get_cgp_outputs(int* best_chromosome, char file[MAX_NUM_LETTERS])
{
	...
	for (fitness_test = 0; fitness_test < num_tests; fitness_test++)
	{
		...
		{	
			...
			if (cgp_outputs[i] < (0.333)) final_outputs[fitness_test][i] = 0;
	...
	for (y = 0; y<num_tests; y++)
	{
		for (x = 0; x < num_outputs; x++)
		{
			...
			if (data_outputs[y][x] != final_outputs[y][x]) error[x] = error[x] + 1;
	...
	for (x = 0; x<num_outputs; x++)fprintf(fo, "For out %d fitness = %lf	errors = %lf  sum of squared error = %lf	total error = %lf\n",x , fit[x], error[x], sq_error[x], tot_error[x]);
