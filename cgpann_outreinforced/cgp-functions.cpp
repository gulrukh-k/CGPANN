...
void read_data(char datafile[MAX_NUM_LETTERS])
{
	...
	else
	{
		...
		for (i = 0; i < num_tests; i++)
		{
			...
			{
				if (j % outs == 0) data_outputs[i][j] = myfscanf(fp);
				else data_outputs[i][j] = data_outputs[i][j - 1];
			}
		}
		fclose(fp);
	}
	...
	int fitness_test)
{
	...
	for (j = 0; j < num_outputs/outs; j++)
	{
		...
		cgp_data = 0;
		for (i = 0; i < outs; i++) cgp_data = cgp_data + output[chromosome[num_genes - num_outputs + j + i]];    /* get output from nodes referenced in output genes */
		cgp_data = (double)cgp_data / outs;
...
void get_cgp_outputs(int* best_chromosome, char file[MAX_NUM_LETTERS])
{
	...
	for (x = 0; x<num_outputs/outs; x++)fprintf(fo, "Nodes used = %d	fitness = %lf	errors = %lf	sum of squared error = %lf	total error = %lf\nfitness percentage = %lf\n positive = %d	negative =%d	ones=%d	zeroes =%d", num_nodes_to_process, fit[x], error[x], sq_error[x], tot_error[x], 100 * (num_tests - error[x])/num_tests, positive[x], negative[x], ones[x], zeroes[x]);
