...
void validate_command_line(int argc, char* argv[], char parfile[], char datafile[])
{
	...
	puts("********* Validating command line arguments to cgpann program *********");
...
//	int     arity, max_arity;
	...
	/* each node is assigned number of inputs * 3 (input, weight and connection) genes */
	num_genes_per_node = (MAX_NUM_INPUTS * 3) + 1;														
...
/* reads input and output data from a file  
	...
	int fitness_test)
{
	...
	for (j = 0; j< num_nodes_to_process; j++)
	{
		...
		{
			if (i % 3 == 0) in[i] = output[chromosome[first_node_gene + i]];
	...
	for (j = 0; j < num_outputs; j++)
	{
		...
		{
			if (cgp_data < (double)i / NUM_CLASSES) cgp_outputs[j] = (double)i / (NUM_CLASSES - 1);
		}*/
		if (cgp_data < 0.5) cgp_outputs[j] = 0.0; else cgp_outputs[j] = 1;
		//if (cgp_data < 0.0) cgp_outputs[j] = -1.0; else cgp_outputs[j] = 1;
	...
	data_type cgp_output)
{
	...
	if (cgp_output == data_output) result = 1.0; else result = 0.0;
#endif
...
{
	return (is_not_output_gene(gene) && (locus % 3 ==1) );

}
int is_connection_gene(int gene, int locus)
{
	return (is_not_output_gene(gene) && (locus % 3 == 2));
		...
		for (row = 0; row<num_rows; row++)
		{
			...
			for (i = 0; i < num_genes_per_node - 1; i++)
			{
				...
				case 0:
					...
					break;
				case 1: 
					chromosome[count + i] = get_weight();// random weight
					break;
				case 2:
					chromosome[count + i] = newrand(2);// connection gene can be 0 or 1
					break;
				default:
					break;
				}				
...
{
	return (int)((num_genes + num_nodes)*per_cent_mutate / 100.0);
}
...
void mutate_a_gene(int*  chromosome)
{
	...
	which_gene = newrand(num_genes + num_inputs);
	...
	if (is_not_output_gene(which_gene))
	{
		...
		/* it is input, weight or connection gene */
		else if (is_weight_gene(which_gene, which_locus)) 	chromosome[which_gene] = get_weight();
		else if (is_connection_gene(which_gene, which_locus)) 	chromosome[which_gene] = !chromosome[which_gene];// for connection gene will be 0 for 1 and 1 for 0
		else
		{
	...
	{
		chromosome[num_genes - 1] = get_output_gene();
	}
}
...
{
	printf("\nGENERATION is %d Best fitness is now %8.5lf which is %8.5lf %%", generation, fit, ((fit * 100)/ num_tests));
}
	...
	int* best_chromosome)
{
	...
	fp = fopen(prog, "a");
	fprintf(fp, "\nGENERATION is %u     Best fitness is now %8.5lf with %8.5lf %%", gen, best_fit, ((best_fit * 100) / num_tests));
	fprintf(fp, "\nThe chromosome is\n");
	fclose(fp);
	...
	char prog[MAX_NUM_LETTERS])
{
	...
	for (gen = 1; gen <= num_generations; gen++)
	{
		...
		if (flag == 1)
		{
			...
			else if (gen < num_generations / 2) per_cent_mutate = 15.0;
			else if (gen < 3 * num_generations / 4) per_cent_mutate = 10.0;
			else per_cent_mutate = 5.0 ;
		}
...
void get_cgp_outputs(int* best_chromosome, char file[MAX_NUM_LETTERS])
{
	...
	double fit[MAX_NUM_OUTPUTS], err, error[MAX_NUM_OUTPUTS], sq_error[MAX_NUM_OUTPUTS], tot_error[MAX_NUM_OUTPUTS];
	...
	for (fitness_test = 0; fitness_test < num_tests; fitness_test++)
	{
		...
		{	
			...
			//if (cgp_outputs[i] < (0.5)) final_outputs[fitness_test][i] = 0;
			////else if (cgp_outputs[i] < (0.666)) final_outputs[fitness_test][i] = 0.5;
			//else final_outputs[fitness_test][i] = 1.0;
			//cgpout2[fitness_test][i] = cgp_outputs[i];// get the value into cgpout2 array
		}
	}
	...
	for (y = 0; y<num_tests; y++)
	{
		for (x = 0; x < num_outputs; x++)
		{
			...
			{
				error[x] = error[x] + 1;
				if (data_outputs[y][x] > final_outputs[y][x]) negative[x] = negative[x] + 1; else positive[x] = positive[x] + 1;
			}
			else if (data_outputs[y][x] == 1) ones[x] = ones[x] + 1; else zeroes[x] = zeroes[x] + 1;

			err = data_outputs[y][x] - final_outputs[y][x];
			fit[x] = fit[x] + correctness_test(data_outputs[y][x] , cgpout2[y][x]);
			sq_error[x] = sq_error[x] + (err * err) / 2;
			tot_error[x] = tot_error[x] + (data_outputs[y][x] - cgpout2[y][x]);
		}
		fprintf(fo, "\t\t\t");
	...
	for (x = 0; x < num_outputs; x++)
	{
		fprintf(fo, "Nodes used = %d	fitness = %lf	errors = %lf	correct = %lf %%\n", num_nodes_to_process, fit[x], error[x], (num_tests - error[x]) * 100 / num_tests);
		fprintf(fo, "%d		%d\n%d		%d", zeroes[x], positive[x], negative[x], ones[x]);
	}
...
This function gets the output of the best chromosome into the global variable array cgpout2. It then prints the cgp out put as well as corressponding data output.
*/
void get_volcano_outputs(int* chromosome1, int* chromosome2, char file[MAX_NUM_LETTERS])
{
	FILE* fo;
	int x, y, i;
	int events, LP, VT;
	int devents, dLP, dVT;
	int			fitness_test, count1, count2, count3, change;
	int			num_nodes_to_process1, num_nodes_to_process2;
	int			nodes_to_process1[MAX_NUM_NODES], nodes_to_process2[MAX_NUM_NODES];
	data_type	cgp_outputs1[MAX_NUM_OUTPUTS], cgp_outputs2[MAX_NUM_OUTPUTS], final_outputs1[MAX_NUM_DATA][MAX_NUM_OUTPUTS], final_outputs2[MAX_NUM_DATA][MAX_NUM_OUTPUTS];
	//double fit[MAX_NUM_OUTPUTS], error[MAX_NUM_OUTPUTS], sq_error[MAX_NUM_OUTPUTS], tot_error[MAX_NUM_OUTPUTS];
	data_type prev_out1, prev_out2;
	count1 =  count2 = count3 = change = 0;
	prev_out1 = prev_out2 = 0.0;
	/* find out how many nodes there are in the phenotype */
	num_nodes_to_process1 = get_nodes_to_process(chromosome1, nodes_to_process1);
	num_nodes_to_process2 = get_nodes_to_process(chromosome2, nodes_to_process2);
	events = LP = VT = 0;
	devents = dLP = dVT = 0;
	fo = fopen(file, "w");
	...
	for (fitness_test = 0; fitness_test < num_tests; fitness_test++)
	{
		decode_cgp(chromosome1, data_inputs, cgp_outputs1,
			num_nodes_to_process1, nodes_to_process1, fitness_test);
		
		decode_cgp(chromosome2, data_inputs, cgp_outputs2,
				num_nodes_to_process2, nodes_to_process2, fitness_test);

		if (fitness_test == 0)
		{
			final_outputs1[fitness_test][0] = 0.0;
			prev_out1 = 0;
			prev_out2 = -10;
		}
		else
		{
			if (cgp_outputs1[0] == prev_out1)
				count1++;
			else
				count1 = 0;
			if (cgp_outputs2[0] == prev_out2)
				count2++;
			else
				count2 = 0;
			if (count1 > 2)
				final_outputs1[fitness_test][0] = cgp_outputs1[0];
			else
				final_outputs1[fitness_test][0] = final_outputs1[fitness_test - 1][0];
			if (count2 > 2)
				final_outputs2[fitness_test][0] = cgp_outputs2[0];
			else
				final_outputs2[fitness_test][0] = final_outputs2[fitness_test - 1][0];
			if ((final_outputs1[fitness_test - 1][0] == 0) && (final_outputs1[fitness_test][0] == 1)) events++;
		}
		if (final_outputs1[fitness_test][0] == 0) final_outputs2[fitness_test][0] = -10;

		if (final_outputs2[fitness_test][0] > -1) count3++; else count3 = 0;
		if (count3 == 2)
		{
			if (final_outputs2[fitness_test][0] == 0) LP++; else VT++;
		}

		if (fitness_test > 0)
		{
			if (data_outputs[fitness_test][0] != data_outputs[fitness_test - 1][0])
			{
				if (data_outputs[fitness_test][0] > 0)
				{
					devents++;
					if (data_outputs[fitness_test][0] < 1.0) dLP++;
					else dVT++;
				}
			}
		}
		...
		prev_out1 = cgp_outputs1[0];
		prev_out2 = cgp_outputs2[0];

		for (i = 0; i < num_outputs; i++)
		{
			cgpout1[fitness_test][i] = cgp_outputs1[i];// get the value into cgpout2 array
			cgpout2[fitness_test][i] = cgp_outputs2[i];
		}
	}
	//prints output to file
	for (y = 0; y<num_tests; y++)
	{
		for (x = 0; x < num_outputs; x++)
		{
			fprintf(fo, "%lf	 %lf		%lf	 %lf\t\t\t", cgpout1[y][x], cgpout2[y][x], final_outputs1[y][x], final_outputs2[y][x]);
		}
		fprintf(fo, "\t\t\t");
		for (x = 0; x<num_outputs; x++)fprintf(fo, " %lf\t\t", data_outputs[y][x]);
		fprintf(fo, "\n");
	}
	for (x = 0; x<num_outputs; x++)fprintf(fo, "Events = %d		LP = %d		VT = %d \t\t\tData Events = %d	Data LP = %d	Data VT = %d",events, LP, VT, devents, dLP, dVT);

	fclose(fo);
}
void run_volcanotest(char datafile[MAX_NUM_LETTERS], char outfile[MAX_NUM_LETTERS])
{
	int* chrom1;
	int* chrom2;
	FILE* fp1;
	...
	int i;
	read_data(datafile);
	chrom1 = create_1d_space(num_genes);
	chrom2 = create_1d_space(num_genes);
	fp1 = fopen("cgp.chr", "r");
	if (!fp1)
	{
		puts("Missing file cgp.chr (contains a chromosome)");
	...
	else
	{
		...
		do
		{
			fscanf(fp1, "%d", &chrom1[i]);
			i++;
		} while (!feof(fp1));

		if (i != num_genes)
		{
			puts("ERROR. Number of genes in cgp1.chr does not match the expected number");
			printf("\nnum_genes required is %d, num_genes read is %d", num_genes, i);
			puts("Check the number of genes in the .par file");
			exit(0);
		}
	...
	fp2 = fopen("cgp2.chr", "r");
	if (!fp2)
	{
		puts("Missing file cgp2.chr (contains a chromosome)");
		exit(1);
	}
	else
	{
		...
		do
		{
			fscanf(fp2, "%d", &chrom2[i]);
			i++;
		} while (!feof(fp2));

		if (i != num_genes)
		{
			puts("ERROR. Number of genes in cgp2.chr does not match the expected number");
			printf("\nnum_genes required is %d, num_genes read is %d", num_genes, i);
			puts("Check the number of genes in the .par file");
			exit(0);
		}
	...
	get_volcano_outputs(chrom1, chrom2, outfile);
	fclose(fp1);
	fclose(fp2);
